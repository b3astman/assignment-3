...
window.onload = (function () {
    ...
    saveButton.onclick = function () { 
        console.log("saving");
        var saveData = {circles: []};
        for (var i = 0; i < it.length; i++) {
            var circle = {x: null, y: null, radius: null, color: null,
                 velocity: {x: null, y: null}};
            circle.x = it[i].x;
            circle.y = it[i].y;
            circle.radius = it[i].radius;
            circle.color = it[i].color;
            circle.velocity.x = it[i].velocity.x;
            circle.velocity.y = it[i].velocity.y;
            if (it[i].removeFromWorld == false) {
                saveData.circles.push(circle);
            }
        }
        socket.emit("save", {studentname: "Bethany Eastman", statename: "aState", data: saveData});
    ...
    loadButton.onclick = function () { 
        console.log("loading");
        socket.emit("load", {studentname: "Bethany Eastman", statename: "aState"});
    };
    ...
    socket.on("load", function (data) {
        ...
        for (var i = 0; i < data.data.circles.length; i++) {
            var circle = new Circle(gameEngine);
            circle.x = data.data.circles[i].x;
            circle.y = data.data.circles[i].y;
            circle.radius = data.data.circles[i].radius;
            circle.color = data.data.circles[i].color;
            circle.velocity.x = data.data.circles[i].velocity.x;
            circle.velocity.y = data.data.circles[i].velocity.y;
            it.push(circle);
            gameEngine.addEntity(circle);
        }
    });
